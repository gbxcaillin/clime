...
- The tabs (Ticks, Adhoc, Tasks, Roles, Hidden, People, Log, Meta) are plain data and can be read, filtered or corrected by hand. Rows that do not make sense are skipped rather than breaking the page. The whole set of tabs is rewritten on every save, so hand edits made between two saves can be lost; pause the tracker or edit through the page for anything important.
